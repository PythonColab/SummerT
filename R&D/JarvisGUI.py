...
lb1 = Label(window, text="JARVIS",height=3)
lb1.grid(column=0, row=0,columnspan=4)
lb2 = Label(window,anchor="nw",text="Click Speak for Starting",height=15,width=65,borderwidth=4, relief="sunken",justify=LEFT)
...
def recordAudio():
    ...
    try:
        ...
        p("Google Speech Recognition could not understand audio")
    except sr.RequestError as e:
        p("Could not request results from Google Speech Recognition service; {0}".format(e))
...
def clicked1(name):
    ...
    data = recordAudio()
    jarvis(data,name)
btn = Button(window, text="SPEAK", command= lambda: clicked1(name),width=50,height=3)
...
btn1 = Button(window, text="Enter", command=clicked,width=30)
